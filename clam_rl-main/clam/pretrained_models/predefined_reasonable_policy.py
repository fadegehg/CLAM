...
for i in range(3):
    save_dict = torch.load('pretrained_models/pretrained/params_ia2c_agent_' + str(i) + '.pt')
    ia2c_agents[i].load_state_dict(save_dict['agent_params']['actor_critic'])
maddpg_agents = [MADDPGNet(16, 5, 64) for _ in range(3)]
for i in range(3):
    save_dict = torch.load('pretrained_models/pretrained/params_maddpg_agent_' + str(i) + '.pt')
...
opp = [after_prey,  after_agent_far, after_agent_close, after_lead]
opp += ia2c_agents
opp += maddpg_agents


# this is our reasonable policy combination index
index_rs = np.array([[0,  3,  3],
                    [0,  1,  2],
                    [7,  8,  9],
                    [0,  0,  0],
                    [4,  5, 6]])

pretrained_agents_rs = []
for i in range(5):
    pretrained_agents_rs.append([opp[index_rs[i][0]], opp[index_rs[i][1]], opp[index_rs[i][2]]])


def get_predator_action_rs(obs, task_id):
    actions = []
    for i, o in enumerate(obs):
        actions.append(pretrained_agents_rs[task_id][i](o))
    return actions