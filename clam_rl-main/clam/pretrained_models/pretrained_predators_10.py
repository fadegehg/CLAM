...
for i in range(3):
    save_dict = torch.load('/home/lou/Data/Liang_13060835/projects/fuzzy_clam_rl/clam/pretrained_models/pretrained/params_ia2c_agent_' + str(i) + '.pt')
    ia2c_agents[i].load_state_dict(save_dict['agent_params']['actor_critic'])
maddpg_agents = [MADDPGNet(16, 5, 64) for _ in range(3)]
for i in range(3):
    save_dict = torch.load('/home/lou/Data/Liang_13060835/projects/fuzzy_clam_rl/clam/pretrained_models/pretrained/params_maddpg_agent_' + str(i) + '.pt')
...
def get_predator_actions(obs, task_id):
    actions = []
