# from https://github.com/uoe-agents/LIAM/blob/main/multi_agent_env/predator_prey.py
import numpy as np
from mpe.core import World, Agent, Landmark, Wall
...
class Scenario(BaseScenario):
    ...
    def agent_reward(self, agent, world):
        ...
        if num_collisions > 1:
            rew -= 1
        if agent.state.p_pos[0] > 0.99 or agent.state.p_pos[0] < -0.99 or agent.state.p_pos[1] < -0.99 or \
                agent.state.p_pos[1] > 0.99:
            rew -= 10
        return rew
